function [p,u,eqn,info] = DFP0P1TPD_PDHG(node,elem,bdFlag,pde,option)
...
res0  = norm([M.*uoldAll + Bp - eqn.f; eqn.Bff*uoldAll  - eqn.gf]);


%% Solve the saddle point problem
alphap = 0.5;
alphau = 0.5; % parameter for nonlinear iteration for u
gamma0 = 0.0001;
Vcycle_num = 0;
...
%     fprintf('Iteration %i \n', ite);
    ...
    % implicit solver using proximal operator, preconditioner is M0
    F_half = uoldAll - alphau*(Bp./repmat(area,2,1)) + alphau*(eqn.f./repmat(area,2,1));
    F_abs = sqrt(F_half(1:NT).^2 + F_half(NT+1:end).^2);
    gamma = (1 + alphau)/2 + (1/2)*sqrt((1+alphau)^2+4*alphau*pde.beta*F_abs);  
    uhalf = repmat((1./gamma),2,1).*F_half;


     dp_tmp = eqn.gf - eqn.Bff*uhalf;
     pnew = poldAll(freep) - alphap*dp_tmp(freep);


     % pnew = poldAll(freep) - alphap* (dp_tmp(freep)./Mp(freep));

     pnewAll =  poldAll;   % boundary values
     pnewAll(freep) = pnew;
    
    unewAll =  uhalf + 0.5*(uhalf - uoldAll);
   ...
   % M. Mass matrix for P0 element
    ...
    res(ite) = norm([M.*unewAll + Bp - eqn.f; eqn.Bff*unewAll  - eqn.gf]);
    ...
    uoldAll = unewAll;
    poldAll = pnewAll;
    theta = 1/(1+ 2*sqrt(gamma0*alphau));
    alphau = theta*alphau;
    alphap = alphap/theta;
...
%% Output
info = struct('itStep', ite ,'res',res(1:ite), ...
